...
sub handler {
  ...
  return $self->_default($event, @_) unless defined $ev; # glob - fix uninit warning

  print STDERR "Trying to handle event '$ev'.\n" if $self->{_debug} && $ev; # glob

  my $handler = undef;
  if ($ev && exists $self->{_handler}->{$ev}) {
    $handler = $self->{_handler}->{$ev};
  } elsif ($ev && exists $_udef{$ev}) {
    $handler = $_udef{$ev};
  } else {
...
sub sl_real {
  ...
  my $rv = $self->ssl ?
      $self->socket->syswrite("$line\015\012") :
...
sub _default {
  ...
  return 1 unless $event->type; # glob - fix uninit warning
