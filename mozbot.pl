...
my $LOGFILEDIR = 'log'; # set this to override the logging output directory
...
if ($LOGGING) {
	...
	$LOGFILEPREFIX = 'log/mozbot';
    mkdir($LOGFILEDIR, 0700); # if this fails for a bad reason, we'll find out during the next line
}

my $inStartup = 1; # let's not write the config file till we're done starting up, k?
my $lastchannel = ""; #startup is done when we join the last channel. Ugh.
...
my $delaytime = 0; # amount of time to wait between outputs
my $recentMessageCountThreshold = 10; # threshold before we stop outputting
my $recentMessageCountPenalty = 5; # if we hit the threshold, bump it up by this much
my $recentMessageCountLimit = 30; # limit above which the count won't go
my $recentMessageCountDecrementRate = 0.5; # how much to take off per $delaytime
...
sub on_connect {
	...
	$lastchannel = @channels[scalar(@channels)-1]; #store the last channel for tracking end of startup
	$self->schedule(300, \&on_join_last_channel); #schedule check that we're done starting up.
...
sub on_join_channel {
	...
	if ($channel eq $lastchannel)
	{
		on_join_last_channel();
	}
}

sub on_join_last_channel
{
	if ($inStartup)
	{
		debug("Completed joining channels.");
		$inStartup = 0; #Done with startup
		save_config_vars([]); #Save config file
	}
}
    ...
    if (@msgqueue > 0) {
        ...
        if (@msgqueue > 0) {
            if ((@msgqueue % 10 == 0) and (time() - $timeLastSetAway > 5 * $delaytime)) {
                ...
                $self->schedule($delaytime * 2, # because previous one counts as message, plus you want to delay an extra bit regularly
                    \&drainmsgqueue);
            } else {
